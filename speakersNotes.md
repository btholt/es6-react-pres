...
This will be used once we get to server-side rendering. Really the only difference is that it has and underscore templating tag in it (more on that later.)
...
- Sweet! After this should see the names of ten different Netflix series. Let's talk about what's new here.
...
- Most of note here would be `componentDidMount` method. This method is a React lifecycle method is called immediately after the component is run for the first time. Why do make the AJAX call only after the component has rendered? Otherwise you're stuck waiting on AJAX calls that block your page from rendering. This gives you a faster perceived loading time. You can do things like throw up spinners to let people know that work is still being done behind the scenes.
- This is going throw some 404s because at first img's source is undefined. Obviously not a good idea, but we'll fix it here in a sec.
...
- Awesome. Should be seeing a bunch show posters now. If you look at your network tab (and you're not using `fake-omdb-client`) you should see a bunch of network requests. This is obviously a very inefficient way for us to request data; we're doing it this way because want the movie posters for each movie. In real life, you'd figure out some better way to batch this. No worries, I talked to owner of OMDb and he says we're cool to make a bunch of requests today to it. I also donated a bit to hopefully offset any costs we incur him.
- Notice we have key as well. React wants to be as efficient as possible. Here we're giving React a unique key so that it knows that when it re-renders, that this is the same object so if it has re-order them, it's not creating and destroying or anything like, but simply re-arranging existing DOM nodes. Plus if you don't give it a key, it gives you noisy console warnings.
...
- ES6 template strings! So rad. Love them. These are already present in other languages but essentially it allows you to avoid the awful syntax of concatenating strings together. The format is having back-ticks instead of single or double quotes. Inside of these template strings, you can use ${} to denote where you want to put any sort of JavaScript expression. In ours, as you can see, we're putting random number between 1 and 5.
...
- Take a look, you should some star ratings using stars. Cool!
...
// end method insdie Header class
...
- Here you see a few commonly observerd patterns in React. First is handling events. It should feel familiar even if you haven't used React before. You declare on the pertitent markup an event handler for a type of event (onClick, onChange, onSubmit, etc.) that maps to an event handler.
...
- If you're familiar with React, you may be curious with the `.bind(this)` on the event handler. React used to do auto-binding of the context for you… sometimes. Most of the time. They no longer do it because the times they didn't autobind for you was really, really confusing. Instead they're making you bind it yourself every time so hopefully you just get in the habit.
...
- Give it a try; now you should be able to change the drop down. You won't see it do anything yet but it is modifying the interal state. If you have the Chrome extension for React, you can investigate the internal state and see it change. Let's go add the next context to see what that looks like.
...
- You should now have an input box that you cannot type in! Yay! This is the same issue we saw the drop down. Let's fix it.
...
- Kinda going all-in at once, since it doesn't really work without all the parts and this isn't a server-side JS workshop so I'm not going to explain in the ins and outs of it.
...
That's it! You should have an React with ES6 app working that is getting server-side pre-rendered! Hooray!
